...
# export EDITOR="$( (which zeditor &> /dev/null && echo zeditor) || echo vim )"
...
alias scratch='clojure -Sdeps "{:deps {clj-scratch {:git/url \"https://github.com/jorinvo/clj-scratch\" :sha \"cec1c443e66eb62a15e7f5de04fb640502c9c02a\"}}}" -m scratch'
...
o() {
  local howtoopen="$(type xdg-open &> /dev/null && echo xdg-open|| echo open)"
  if [ "$1" = "" ]; then
    $howtoopen .
  else
    $howtoopen "$1"
...
# Only start ssh agent when in interactive shell
# Otherwise we get a password prompt on system login
if [[ $- == *i* ]]; then
